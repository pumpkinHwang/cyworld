...
export default class ImageTools {
    ...
    resize = (file, maxDimensions) => new Promise((resolve, reject) => {
        ...
        image.onload = () => {
            ...
            if (width < 320 || height < 720) {
                reject('u_all_idverification_resolution_too_small')
            }
            ...
            EXIF.getData(image, () => {
                ...
                const imageCanvas = this.drawImageToCanvas(image, 1, 0, 0, width, height, 'contain');
                if (hasToBlobSupport) imageCanvas.toBlob(blob => resolve(blob), file.type, 1);
    ...
    convertToImage = (blob, fileName) => {
        return new File([blob], fileName, { lastModified: new Date() });
    }

    imageExist = (image_url) => {

        $.get(image_url + '?check_image=exist').done((res) => {
            return true
        }).fail((res) => {
            return false
        })
